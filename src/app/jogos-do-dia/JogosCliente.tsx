...
const AiAnalysisModal = ({ fixtureData, analysisData, pageData, onClose }: any) => {
    ...
    const generateDetailedAnalysis = () => {
        if (!analysisData || !pageData.standings[league.id] || !analysisData.homeTeamForm) {
        ...
        <div className="bg-white p-4 rounded-b-lg text-black border-t-2 border-blue-600 animate-pulse">
            ...
            <div className="space-y-4">
                ...
                <div className="flex -mb-px border-b border-gray-200">
                    <div className="w-20 h-8 bg-gray-200 rounded-t-lg mr-2"></div>
                    <div className="w-20 h-8 bg-gray-200 rounded-t-lg mr-2"></div>
                    <div className="w-20 h-8 bg-gray-200 rounded-t-lg mr-2"></div>
                </div>
                <div className="pt-4 min-h-[250px] space-y-3">
    ...
    <div className="bg-white p-4 rounded-lg shadow-md h-fit space-y-4 mb-6">
            ...
            <div>
                ...
                <div className="relative mt-1">
                    ...
                    <input
                        ...
                        value={searchQuery}
                        type="text"
                        placeholder="Time ou competição..."
                        className="w-full bg-gray-50 border border-gray-300 text-black rounded-md p-2 pl-10 pr-10 focus:ring-1 focus:ring-blue-500"
                    />
                    {searchQuery && (
                        <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-black transition-colors">
                            <X size={18} />
                        </button>
                    )}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end pt-4 border-t">
            <div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                    <button onClick={() => setGroupBy('league')} className={`p-2 rounded-md text-sm font-bold transition-colors ${groupBy === 'league' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-black hover:bg-gray-300'}`}>
                        Torneio
                    </button>
                    <button onClick={() => setGroupBy('time')} className={`p-2 rounded-md text-sm font-bold transition-colors ${groupBy === 'time' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-black hover:bg-gray-300'}`}>
                        Horário
                    </button>
                </div>
            </div>
...
export default function JogosCliente({ initialData }: { initialData: any }) {
    ...
    const fetchAnalysis = async (gameData: any) => {
        ...
        if (analysisCache[fixture.id]) return;
    ...
    const filteredFixtures = useMemo(() => {
        ...
        let baseFilteredGames = pageData?.fixtures
        ...
        if (selectedLeague !== 'all') {
            baseFilteredGames = baseFilteredGames.filter((f: any) => f.league.id.toString() === selectedLeague);
        }

        if (searchQuery.length > 2) {
            const lowerCaseQuery = searchQuery.toLowerCase();
            baseFilteredGames = baseFilteredGames.filter((f: any) =>
    ...
    }, [pageData, activeDate, searchQuery, selectedLeague, groupBy]);
        ...
        <>
            ...
            <main className="space-y-3">
                {filteredFixtures.length > 0 ? (
                    filteredFixtures.map(([groupName, groupData]: [string, any]) => (
                        <div key={groupName} className="bg-white rounded-lg shadow-md">
                            <h3 className="text-sm font-bold text-black p-3 border-b flex items-center gap-2">
                                {groupData.games[0]?.league?.logo && (
                                    <Image src={groupData.games[0].league.logo} alt={groupName} width={16} height={16} />
                                )}
                                {groupName}
                            </h3>
                            <div className="p-1">
                                {groupData.games.map((game: any) => {
                                    const isExpanded = expandedFixtureId === game.fixture.id;
                                    const isLoading = loadingFixtureId === game.fixture.id;
                                    const currentAnalysisData = analysisCache[game.fixture.id];

                                    return (
                                        <div key={game.fixture.id}>
                                            <div onClick={() => handleSelectFixture(game.fixture.id, game)} className={`grid grid-cols-[60px_1fr_1fr_auto] items-center p-2 rounded-md cursor-pointer transition-colors ${isExpanded ? 'bg-blue-100 rounded-b-none' : 'hover:bg-gray-100'}`}>
                                                ...
                                                <ChevronDown className={`ml-auto text-gray-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`} size={20} />
                                            </div>

                                            {isExpanded && (
                                                isLoading ? (
                                                    <AnalysisPanelSkeleton fixtureData={game} />
                                                    ...
                                                    <div className="text-center text-red-500 p-6 bg-red-50 rounded-b-lg border-t-2 border-red-500">
