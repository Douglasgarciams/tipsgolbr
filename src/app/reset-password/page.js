...
import { useRouter, useSearchParams } from 'next/navigation'; // Importa useSearchParams para ler a URL
...
export default function ResetPasswordPage() {
  ...
  const [token, setToken] = useState(null); // Estado para armazenar o token da URL
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(data.message); // Mensagem de sucesso da API
      setPassword('');
      setConfirmPassword('');
  ...
  return (
    <div className="min-h-screen login-background flex items-center justify-center">
      <div className="bg-gray-800 bg-opacity-90 p-8 rounded-lg shadow-lg w-full max-w-sm text-white">
        ...
        {!token && !error && (
            <p className="text-sm text-center text-gray-300 mb-6">Carregando...</p>
        )}

        {token && !message && ( // Só mostra o formulário se tiver token e não tiver mensagem de sucesso
